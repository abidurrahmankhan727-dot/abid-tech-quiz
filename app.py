...
# কুইজ শেষ হওয়ার পর রিভিউ সেকশন
    ...
    for i, ans in enumerate(st.session_state.user_answers):
        if ans['is_correct']:
            st.markdown(f"""<div class="correct-box">
                ...
                ✅ Your Answer: {ans['user_choice']} (Correct)
                </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div class="wrong-box">
                <b>Q{i+1}: {ans['question']}</b><br>
                ❌ Your Answer: {ans['user_choice']}<br>
                ✔️ Correct Answer: {ans['correct_answer']}
                </div>""", unsafe_allow_html=True)
    ...
    st.warning("👈 Use the sidebar to click 'Start New Quiz'!")
